	...
	: AfiducialNavigation(detectorType, visionSysVector, detectorConfigFile)
{
	...
	Board = new CustomMultipleBoard(fiducialDetector->idsList.size());
	...
	for (int i = 0; i < markerCount; i++)
    {
		...
		Board->addMarkerToMap(markerId, markerParams[8], markerParams[2],
	...
	for (unsigned int i = 0; i < fiducialDetector->visionSysVector.size(); i++)
	{
		if (fiducialDetector->detectorStatus)
		{
				...
				if (!Board->objPointsMat.empty())
				{
					...
					#ifdef VISUALIZATION
						VisualizationOnImage::drawMarkersAndAxes(fiducialDetector->visionSysVector[i]->image, fiducialDetector->cornersList[i],
																 fiducialDetector->idsList[i], rvec, tvec,
																 fiducialDetector->visionSysVector[i]->cameraMatrix,
																 fiducialDetector->visionSysVector[i]->distCoeffs, axesSize);
					#endif
					status = 1;
				}
...
void	FiducialSlamNavigation::updateIdsDetectedTime(const unsigned int &cameraId)
{
	...
	for (unsigned int i = 0; i < Board->unknownIds.size(); i++)
	{
		...
		VectorXd_t markerPoseInMap = transformPoseToMap(rvecs[i], tvecs[i], cameraId);
...
VectorXd_t   FiducialSlamNavigation::transformPoseToMap(cv::Vec3d &rvecMarker, cv::Vec3d &tvecMarker, const unsigned int &cameraId)
{
	...
	VectorXd_t markerPoseCam = fiducialDetector->visionSysVector[cameraId]->getCamPosition(rvecMarker, tvecMarker);
